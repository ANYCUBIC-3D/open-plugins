...
static bool derive_key(const std::string &password, const unsigned char *salt,
...
std::string aesEncrypt(const std::string &plaintext,
  ...
  do {
    ...
    // 处理最后的数据块
    if (EVP_EncryptFinal_ex(ctx, buffer + SALT_SIZE + IV_SIZE + len, &len) != 1)
      break;
    total_len += len;
...
std::string aesDecrypt(const std::string &ciphertext,
  ...
  do {
    ...
    if (EVP_DecryptUpdate(ctx, buffer, &len, encrypted,
                          ciphertext.size() - SALT_SIZE - IV_SIZE) != 1)
      break;
    int total_len = len;
