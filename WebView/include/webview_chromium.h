...
public:
           ...
           const wxString& name = wxWebViewNameStr)
   {
    ...
    virtual void RegisterDownloadHandler(wxSharedPtr<wxWebViewDownloadHandler> handler) override;
    // Chromium-specific functions.
    void SetRoot(const wxFileName& rootDir);
    // 0 renderer,1 browser
    bool SendProcessMessage(int process_id, const wxString& name, const std::vector<wxVariant> &args);
...
public:
    ...
    bool add_extra_info(const wxString& key, const wxString& value);
    WX_DECLARE_STRING_HASH_MAP(wxString, extraInfo);
    // if non-empty, specifies extra info to the browser creator
...
public:
    ...
    std::vector<wxVariant> GetMessageArguments(void) const;
