...
#include "msw/webview_edge.h"

#if USE_WEBVIEW && USE_WEBVIEW_EDGE
...
#include "wx/uri.h"
#include "private/jsscriptwrapper.h"
#include "wx/private/json.h"

#include "wx/msw/private.h"
#include "wx/msw/private/cotaskmemptr.h"

#include "msw/private/comstream.h"
#include "msw/private/webview_edge.h"
...
public:
    ...
    virtual bool ClearCookies(void) override {
        m_clearCokies = true;
        return true; 
    }
    void ClearCookies(wxCOMPtr<ICoreWebView2>&webview){
        if(!m_clearCokies)
            return;
#define CHECK_FEATURE_RETURN(feature)                                          \
{                                                                            \
    if (!feature) {                                                            \
    return ;                                                             \
    }                                                                          \
...
{                                                                            \
    ...
    return;                                                            \
        ...
        wxCOMPtr<ICoreWebView2_13> webView2_13;
        auto hr = webview->QueryInterface(&webView2_13);
        CHECK_FAILURE(hr);
        wxCOMPtr<ICoreWebView2Profile> webView2Profile;
        CHECK_FAILURE(webView2_13->get_Profile(&webView2Profile));
        CHECK_FEATURE_RETURN(webView2Profile);
        wxCOMPtr<ICoreWebView2Profile2> webView2Profile2;
        hr = webView2Profile->QueryInterface(&webView2Profile2);
        CHECK_FAILURE(hr);
        
        // Clear the browsing data from the last hour.
        double endTime = (double)time(nullptr);
        double startTime = 0;
        hr = webView2Profile2->ClearBrowsingDataInTimeRange(
            COREWEBVIEW2_BROWSING_DATA_KINDS_COOKIES, startTime, endTime,
            Microsoft::WRL::Callback<ICoreWebView2ClearBrowsingDataCompletedHandler>(
                [](HRESULT error) -> HRESULT {
                    // TODO: handler result
                    // emit clearBrowsingData(SUCCEEDED(error));
                    // wxCommandEvent event(EVT_WEBVIEW_CLEAR_FINISH);
...
wxWebViewEdgeImpl::~wxWebViewEdgeImpl()
{
    if (m_webView)
    {
        ...
        wxCOMPtr<ICoreWebView2_14> webView2_14;
        if (auto hr = m_webView->QueryInterface(IID_PPV_ARGS(&webView2_14));SUCCEEDED(hr) && webView2_14) {
            webView2_14->remove_ServerCertificateErrorDetected(m_serverCertificateErrorToken);
        }
...
bool wxWebViewEdgeImpl::Create()
{
    ...
    m_pendingAccessToDevToolsEnabled = -1; ///< 交给应用去开关
...
HRESULT wxWebViewEdgeImpl::OnDownloadStarting(ICoreWebView2 *WXUNUSED(sender),ICoreWebView2DownloadStartingEventArgs *args){
    ...
    wxCOMPtr<ICoreWebView2DownloadOperation> download;
    hr = args->get_DownloadOperation(&download);
    if(FAILED(hr)){
        return S_OK;
    }

    wxCoTaskMemPtr<wchar_t> uri;
    hr = download->get_Uri(&uri);
    if(FAILED(hr)){
        return S_OK;
    }
    wxString strUri = wxString(uri);
    wxWebViewEvent evt(wxEVT_WEBVIEW_DOWNLOAD_START, m_ctrl->GetId(), strUri, "", wxWEBVIEW_NAV_ACTION_OTHER);
    m_ctrl->GetEventHandler()->ProcessEvent(evt);
    if(!evt.IsAllowed()){
        args->put_Cancel(TRUE);///< 取消下载
        return S_OK;
    }

    INT64 totalBytesToReceive = 0;
    hr = download->get_TotalBytesToReceive(&totalBytesToReceive);
    if(FAILED(hr)){
        return S_OK;
    }


    wxCoTaskMemPtr<wchar_t> resultFilePath;
    hr = args->get_ResultFilePath(&resultFilePath);
    if(FAILED(hr)){
        return S_OK;
    }

    wxWebViewEvent startEvent(wxEVT_WEBVIEW_DOWNLOAD_SUGGESTED,  m_ctrl->GetId(),strUri, wxString(resultFilePath), wxWEBVIEW_NAV_ACTION_OTHER);
    startEvent.SetInt(static_cast<int>( totalBytesToReceive));
    startEvent.Veto();//先标记成拒绝
    if (m_ctrl && m_ctrl->GetEventHandler())
        m_ctrl->GetEventHandler()->ProcessEvent(startEvent);

    if(startEvent.IsAllowed()){
        wxString strResultFilePath  =  startEvent.GetString();
        hr = args->put_ResultFilePath(strResultFilePath.wc_str());

        if(SUCCEEDED(hr)){
            hr = download->add_BytesReceivedChanged(
                Microsoft::WRL::Callback<ICoreWebView2BytesReceivedChangedEventHandler>(this,&wxWebViewEdgeImpl::OnBytesReceivedChanged).Get(),
                &m_bytesReceivedChangedToken);
            if(FAILED(hr)){
                wxLogApiError("add_BytesReceivedChanged", hr);
            }
            hr = download->add_StateChanged(
                    Microsoft::WRL::Callback<ICoreWebView2StateChangedEventHandler>(this,&wxWebViewEdgeImpl::OnStateChanged).Get(),
                    &m_stateChangedToken);
            if(FAILED(hr)){
                wxLogApiError("add_StateChanged", hr);
            }
        }
    }else{
        args->put_Cancel(TRUE);///< 取消下载
...
HRESULT wxWebViewEdgeImpl::OnBytesReceivedChanged(ICoreWebView2DownloadOperation* download, IUnknown*WXUNUSED(args)) { 
    // Here developer can update UI to show progress of a download
    // using `download->get_BytesReceived` and
    // `download->get_TotalBytesToReceive`.
    INT64 BytesToReceive = 0;
    auto hr = download->get_BytesReceived(&BytesToReceive);
    if(FAILED(hr)){
        return S_OK;
    }
    return S_OK;
}
HRESULT wxWebViewEdgeImpl::OnStateChanged(ICoreWebView2DownloadOperation* download, IUnknown*WXUNUSED(args)) {
    COREWEBVIEW2_DOWNLOAD_STATE downloadState;
    auto hr = download->get_State(&downloadState);
    if(FAILED(hr)){
        return S_OK;
    }
    ...
    case COREWEBVIEW2_DOWNLOAD_STATE_COMPLETED:
        {
            ...
            wxCoTaskMemPtr<wchar_t> resultFilePath;
            hr = download->get_ResultFilePath(&resultFilePath);
            if(FAILED(hr)){
                return S_OK;
            }
            wxWebViewEvent event(wxEVT_WEBVIEW_DOWNLOAD_FINISHED, m_ctrl->GetId(), wxString(uri), wxString(resultFilePath), wxWEBVIEW_NAV_ACTION_OTHER);
            if (m_ctrl && m_ctrl->GetEventHandler())
...
HRESULT wxWebViewEdgeImpl::OnWebViewCreated(HRESULT result, ICoreWebView2Controller* webViewController)
{
    ...
    // 添加忽略SSL证书错误的处理
    wxCOMPtr<ICoreWebView2_14> webView2_14;
    if (hr = m_webView->QueryInterface(IID_PPV_ARGS(&webView2_14));SUCCEEDED(hr) && webView2_14) {
        webView2_14->add_ServerCertificateErrorDetected(
            Callback<ICoreWebView2ServerCertificateErrorDetectedEventHandler>(
                [](ICoreWebView2* sender, ICoreWebView2ServerCertificateErrorDetectedEventArgs* args) -> HRESULT
                {
                    // 忽略所有SSL证书错误
    ...
    //  clear browsing data
    static_cast<wxWebViewConfigurationImplEdge*>(m_config.GetImpl())->ClearCookies(baseWebView);

    m_ctrl->NotifyWebViewCreated();

    wxCOMPtr<ICoreWebView2_13> webView2_13;
    if(hr = m_webView->QueryInterface(IID_PPV_ARGS(&webView2_13));SUCCEEDED(hr) && webView2_13){
        wxCOMPtr<ICoreWebView2Profile> profile;
        if(hr = webView2_13->get_Profile(&profile); SUCCEEDED(hr) && profile){
            wxString tempDir = wxStandardPaths::Get().GetTempDir();
            profile->put_DefaultDownloadFolderPath(tempDir.wc_str());
        }
    }
    ...
    wxCOMPtr<ICoreWebView2_4> webView2_4;
    hr = m_webView->QueryInterface(IID_PPV_ARGS(&webView2_4));
    if(SUCCEEDED(hr)){
        hr = webView2_4->add_DownloadStarting(
            Callback<ICoreWebView2DownloadStartingEventHandler>(this,&wxWebViewEdgeImpl::OnDownloadStarting).Get(),&m_downloadStartingToken
        );
        if(FAILED(hr)){
            wxLogApiError("WebView2::WebViewCreated (add_DownloadStarting):%ld", hr);
        }
    }
...
void wxWebViewEdge::RunScriptAsync(const wxString& javascript, void* clientData) const
{
        ...
        [this, clientData](HRESULT error, PCWSTR result) -> HRESULT
    {
        ...
        if (error == S_OK)
        {
            ...
            SendScriptResult(clientData, true, scriptDecodedResult);
        }
        else
        {
...
#endif // USE_WEBVIEW && USE_WEBVIEW_EDGE
