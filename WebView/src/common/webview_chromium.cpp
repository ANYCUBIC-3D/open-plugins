...
#if USE_WEBVIEW && USE_WEBVIEW_CHROMIUM
...
#ifdef __WXMSW__
#include "msw/private.h"
#endif
#include "private/webview.h"
...
#ifdef __WXOSX__
#include "osx/private/webview_chromium.h"
#endif
...
{
public:
    wxWebViewChromiumHandlerRequest(CefRefPtr<CefRequest> request):
        m_request(request),m_data(nullptr)
    { }

    ~wxWebViewChromiumHandlerRequest()
    { wxDELETE(m_data); }

    virtual wxString GetRawURI() const override
    {
        return std::string(m_request->GetURL());
    }
    ...
    virtual wxInputStream* GetData() const override
    {
        ...
        CefRefPtr<CefPostData> data = m_request->GetPostData();
        if(data->GetElementCount()!=1){
            // unsupported
            return nullptr;
        }
        CefPostData::ElementVector element;
        data->GetElements(element);
        size_t bytes = element[0]->GetBytesCount();
        std::vector<char> buffer(bytes);
        element[0]->GetBytes( bytes,buffer.data());
        m_data = new wxMemoryInputStream (buffer.data(), bytes);
        return m_data;
    }

    virtual wxString GetMethod() const override
    { return std::string(m_request->GetMethod()); }

    virtual wxString GetHeader(const wxString& name) const override
    {
        return std::string(m_request->GetHeaderByName(name.utf8_string()));
    }
    virtual void SetURI(const wxString& uri) override{
        if (uri.empty())
                return;
        m_request->SetURL(uri.utf8_string());
    }
    virtual void SetHeader(const wxString& name, const wxStringList& values)override {
        CefRequest::HeaderMap headers;
        m_request->GetHeaderMap(headers);
        headers.erase(name.utf8_string());
        for(auto value:values){
            headers.insert(std::make_pair<CefString, CefString>(name.utf8_string(),wxString( value).utf8_string()));
        }
        m_request->SetHeaderMap(headers);
    }
    CefRefPtr<CefRequest>  m_request;
    mutable  wxMemoryInputStream  *m_data;
};

class  wxWebViewChromiumHandlerResponse: public wxWebViewHandlerResponse
{
public:
    wxWebViewChromiumHandlerResponse():
    m_finish(false), m_status(200)
    {
    }
    ...
    { m_mimeType  = contentType.utf8_string(); }

    virtual void SetHeader(const wxString& name, const wxString& value) override
    {
        m_headers.insert(std::make_pair<CefString, CefString>(name.utf8_string(),value.utf8_string()));
    }
    ...
    virtual void Finish(wxSharedPtr<wxWebViewHandlerResponseData> data) override
    {
        ...
        size_t size   =  stream->TellI();
        stream->SeekI(0);
        std::vector<char> buffer(size);
        stream->ReadAll(buffer.data(), size);
        m_stream = CefStreamReader::CreateForData(buffer.data(), size);
    }
    ...
    CefStreamResourceHandler * CreateResourceHandler() const{
        return new CefStreamResourceHandler(m_status, m_status_text, m_mimeType, m_headers,m_stream);
    }

    bool                       m_finish;
    int                        m_status;
    CefString             m_status_text;
    CefString             m_mimeType;
    CefRequest::HeaderMap m_headers;
    CefRefPtr<CefStreamReader> m_stream;
};

class wxWebViewChromiunProvider:public CefResourceManager::Provider
{
public:
    wxWebViewChromiunProvider(wxSharedPtr<wxWebViewHandler> handler):m_handler(handler){}
    ~wxWebViewChromiunProvider(){}
    virtual bool OnRequest(scoped_refptr<CefResourceManager::Request> request) override{
        if(m_handler == nullptr ){
            // 这个有点问题
            return false;
        }
        wxWebViewChromiumHandlerRequest req(request->request());
        auto URL = req.GetRawURI();
        wxURI uri(URL);
        if(uri.GetServer().CmpNoCase(m_handler->GetVirtualHost())!=0){
            // 不是要处理的域名
            return false;
        }
        auto pResponse = new wxWebViewChromiumHandlerResponse();
        wxSharedPtr<wxWebViewHandlerResponse> res(pResponse);
        m_handler->StartRequest(req, res);
        if (pResponse->IsFinish()){
            request->Continue(pResponse->CreateResourceHandler());
            return true;
        }
        return false; // 继续处理
...
struct ImplData
{
    ...
    // 
    wxSharedPtr<wxWebViewDownloadHandler> m_downloadHandler;
    wxSharedPtr<wxWebViewHandler>         m_handler;
};
...
public:
    ...
    explicit ClientHandler(wxWebViewChromium& webview,wxCEF::ImplData*implData) : 
    m_webview{webview},m_implData(implData) {}
           ...
           const wxString& name)
{
    ...
    m_clientHandler = new ClientHandler{*this,m_implData};
...
bool wxWebViewChromium::DoCreateBrowser(const wxString& url)
{
    ...
    CefRefPtr<CefDictionaryValue> extraInfo =  nullptr;
    if(!configChrome->m_extraInfo.empty()){
      extraInfo =  CefDictionaryValue::Create();
      for(auto &[key,value]:configChrome->m_extraInfo){
        extraInfo->SetString(key.ToStdString(), value.ToStdString());
      }
    }
...
#else
    auto name = app->argv[0].ToStdString();
    char keychain[] = "--use-mock-keychain";
    char* argv[] = {const_cast<char*>( name.c_str()), keychain};
    int         argc   = 2;
...
void wxWebViewChromium::RegisterHandler(wxSharedPtr<wxWebViewHandler> handler)
{
   ...
   m_implData->m_handler = handler;
}

void wxWebViewChromium::RegisterDownloadHandler(wxSharedPtr<wxWebViewDownloadHandler> handler) {
   m_implData->m_downloadHandler = handler;
}
...
bool wxWebViewChromium::SendProcessMessage(int process_id, const wxString& name, const std::vector<wxVariant>& args) {
    CefProcessId processId = CefProcessId::PID_RENDERER;
    if (process_id == 1) {
        processId =  CefProcessId::PID_BROWSER;
    }
      // Create the message object.
    CefRefPtr<CefProcessMessage> msg = CefProcessMessage::Create(name.ToStdString());
    auto                         vals = msg->GetArgumentList();
    vals->SetSize(args.size());
    int idx = 0;
    for (auto arg : args) {
        wxString type = arg.GetType();
        // // e.g. "string", "bool", "stringlist", "list", "double", "long"
        if(type == wxT("string")){
            vals->SetString(idx++, arg.GetString().ToStdString());
        } else if (type == wxT("bool")) {
            vals->SetBool(idx++, arg.GetBool());
        } else if (type == wxT("double")) {
            vals->SetDouble(idx++, arg.GetDouble());
        } else if (type == wxT("long")) {
            vals->SetInt(idx++, arg.GetLong());
        }else {
            assert(false);
            idx++;
        }
    }
    m_clientHandler->GetBrowser()->GetMainFrame()->SendProcessMessage(processId, msg);
    return true;
}
...
{
    while(m_implData->m_handler != nullptr ){
        wxWebViewChromiumHandlerRequest req(request);
        auto URL = req.GetRawURI();
        wxURI uri(URL);
        if(uri.GetServer().CmpNoCase(m_implData->m_handler->GetVirtualHost())!=0){
            // 不是要处理的域名
             break;
        }
        wxSharedPtr<wxWebViewHandlerResponse> res;
        m_implData->m_handler->StartRequest(req, res);
        return RV_CONTINUE;
    }
    if ( !m_resourceManager )
        return RV_CONTINUE;
...
void wxCEF::ClientHandler::OnBeforeDownload(CefRefPtr<CefBrowser>                browser,
                                            CefRefPtr<CefDownloadItem>           download_item,
                                            const CefString&                     suggested_name,
                                            CefRefPtr<CefBeforeDownloadCallback> callback)
{
    if(m_implData->m_downloadHandler == nullptr){
        return;
    }
    wxString download_path;
    bool     saveas = false;
    m_implData->m_downloadHandler->OnDownloadRequested(std::string( download_item->GetOriginalUrl()),std::string( suggested_name),
                                                download_path,saveas );
    if(download_path.empty()){
        return;
    }
    callback->Continue(download_path.utf8_string(), saveas);
}

void wxCEF::ClientHandler::OnDownloadUpdated(CefRefPtr<CefBrowser>              browser,
                                             CefRefPtr<CefDownloadItem>         download_item,
                                             CefRefPtr<CefDownloadItemCallback> callback)
{
    if(m_implData->m_downloadHandler == nullptr || m_implData->m_downloadHandler->IsCancelled()){
        callback->Cancel();
        return;
    }
    std::string url = download_item->GetOriginalUrl();
    if( download_item->IsComplete()){
        std::string file_name = download_item->GetFullPath();
        m_implData->m_downloadHandler->OnDownloadFinished(url,file_name);
    }else{
        m_implData->m_downloadHandler->OnDownloadUpdated(url, download_item->GetReceivedBytes(), download_item->GetTotalBytes(), download_item->GetCurrentSpeed());
    }
}


bool wxCEF::ClientHandler::OnCertificateError(CefRefPtr<CefBrowser> browser,
        cef_errorcode_t cert_error,
        const CefString& request_url,
        CefRefPtr<CefSSLInfo> ssl_info,
        CefRefPtr<CefCallback> callback) 
                                   ...
                                   CefRefPtr<CefCallback> callback)
{
    ...
    std::unique_lock<std::mutex> lock_scope(m_lock);
...
public:
    ...
    {
        AddEntryHook(&wxWebViewChromiumEntry::Hook);
    }

private:
    static int Hook(int argc,char*argv[])
    {
        ...
        {
            constexpr const char* TYPE_OPTION = "--type=";
            if ( strncmp(argv[n], TYPE_OPTION, strlen(TYPE_OPTION)) == 0 )
            {
...
public:
    ...
    virtual wxVersionInfo GetVersionInfo(wxVersionContext context) override
    {
        ...
        return wxVersionInfo("CEF", major, minor, micro);
    }
...
std::vector<wxVariant> wxWebViewChromiumEvent::GetMessageArguments(void) const {
    auto args = m_message->GetArgumentList();

    std::vector<wxVariant> retvals;
    if (args->GetSize() == 0) {
        return retvals;
    }
    for(int i=0;i<args->GetSize();i++){
        auto v = args->GetValue(i);
        switch ( v->GetType() )
        {
        case CefValueType::VTYPE_STRING:
            retvals.push_back(wxVariant(v->GetString().ToWString()));
            break;
        case CefValueType::VTYPE_INT:
            retvals.push_back(wxVariant(v->GetInt()));
        break;
        case CefValueType::VTYPE_DOUBLE:
            retvals.push_back(wxVariant(v->GetDouble()));
            /* code */
            break;
        case CefValueType::VTYPE_BOOL:
            retvals.push_back(wxVariant(v->GetBool()));
            /* code */
            break;
        default:
            retvals.push_back(wxVariant());
            break;
        }
    }
...
#endif // USE_WEBVIEW && USE_WEBVIEW_CHROMIUM

bool wxWebViewConfigurationChromium::add_extra_info(const wxString& key, const wxString& value) {
    m_extraInfo.clear();
    auto ret =     m_extraInfo.insert({key, value});
    return ret.second;
}
