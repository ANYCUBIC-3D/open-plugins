...
- (id)initWithWxWindow: (wxWebViewWebKit*)inWindow ;
- (void)updateDownloadDelegate:(WebViewDownloadDelegate*)delegate;
@end
...
public:
    ...
    virtual bool ClearCookies(void)override{
        if( [m_webViewConfiguration.websiteDataStore  isPersistent]){
            NSSet<NSString *> *dataTypes = [WKWebsiteDataStore allWebsiteDataTypes];
            // 筛选出 Cookies 数据类型
            NSMutableArray<NSString *> *cookieDataTypes = [NSMutableArray array];
            for (NSString *dataType in dataTypes) {
                if ([dataType isEqualToString:WKWebsiteDataTypeCookies]) {
                    [cookieDataTypes addObject:dataType];
                }
            }
            [m_webViewConfiguration.websiteDataStore removeDataOfTypes:[NSSet setWithArray:cookieDataTypes] modifiedSince:[NSDate dateWithTimeIntervalSince1970:0] completionHandler:^(void) {
                                 ...
                                 const wxString& name)
{
    ...
    WebViewDownloadDelegate* dDelegate =[[WebViewDownloadDelegate alloc] initWithWxWindow: this];
    // Register event listener interfaces
    WebViewNavigationDelegate* navDelegate =
            [[WebViewNavigationDelegate alloc] initWithWxWindow: this];
    
    [navDelegate updateDownloadDelegate:  dDelegate];
...
wxWebViewWebKit::~wxWebViewWebKit()
{
    ...
    [m_webView setUIDelegate: nil];
        ...
        if ( WX_IS_MACOS_AVAILABLE(10, 13) )
        {
            ...
            for (const auto& kv : m_handlers)
            {
                id<WKURLSchemeHandler> handler = [webViewConfig urlSchemeHandlerForURLScheme:wxCFStringRef(kv.first).AsNSString()];
                if ([handler isKindOfClass:[WebViewCustomProtocol class]]) {
                    [(WebViewCustomProtocol*)handler releaseHandler];
                }
            }
        }
#endif // macOS 10.13+

    if(m_navigationDelegate!=nullptr){
...
+ (BOOL)yyhandlesURLScheme:(NSString *)urlScheme {
    if ([urlScheme isEqualToString:@"http"] || [urlScheme isEqualToString:@"file"]) {
        return NO;  //这里让返回NO,应该是默认不走系统断言或者其他判断啥的
    } else {
        return [WXWKWebView yyhandlesURLScheme:urlScheme];
    }
}
...
{
    wxWebViewEvent event(wxEVT_WEBVIEW_DOWNLOAD_FINISHED,
                        webKitWindow->GetId(),
                        wxCFStringRef::AsString(download.originalRequest.URL.absoluteString),
...
{
    wxWebViewEvent startEvent(wxEVT_WEBVIEW_DOWNLOAD_SUGGESTED,
                        webKitWindow->GetId(),
                        wxCFStringRef::AsString(response.URL.absoluteString),
                        wxCFStringRef::AsString(suggestedFilename), wxWEBVIEW_NAV_ACTION_OTHER);
    startEvent.SetInt(static_cast<int>( response.expectedContentLength));
    startEvent.Veto();//先标记成拒绝
    if (webKitWindow && webKitWindow->GetEventHandler())
        webKitWindow->GetEventHandler()->ProcessEvent(startEvent);

    if(startEvent.IsAllowed()){
        // 决定下载文件的保存路径
        NSURL *destinationURL = [NSURL fileURLWithPath:wxCFStringRef(startEvent.GetString()).AsNSString()];
        // 调用 completionHandler 方法，将下载文件的保存路径传递给 WebKit
...
- (void)download:(WKDownload *)download didFailWithError:(NSError *)error resumeData:(NSData *) resumeData
{
    ...
    wxWebViewEvent event(wxEVT_WEBVIEW_ERROR,
                        webKitWindow->GetId(),
                        wxCFStringRef::AsString(download.originalRequest.URL.absoluteString),
                        "download", wxWEBVIEW_NAV_ACTION_OTHER);
    event.SetString(wxCFStringRef::AsString(error.localizedDescription));
...
-(void)updateDownloadDelegate: (WebViewDownloadDelegate*)inDelegate
{
    downloadDelegate = inDelegate;
}
...
- (void)webView:(WKWebView *)webView navigationAction:(WKNavigationAction *)navigationAction didBecomeDownload:(WKDownload *)download {
    ...
    wxWebViewEvent startEvent(wxEVT_WEBVIEW_DOWNLOAD_START,
                        webKitWindow->GetId(),
...
//     if( [download.originalRequest.URL.absoluteString hasSuffix:@".gcode"] || [download.originalRequest.URL.absoluteString hasSuffix:@".3mf"] || [download.originalRequest.URL.absoluteString hasSuffix:@".stl"])
//     {
//         wxWebViewEvent startEvent(wxEVT_WEBVIEW_DOWNLOAD_START,
//                         webKitWindow->GetId(),
//                         wxCFStringRef::AsString( download.originalRequest.URL.absoluteString ),
