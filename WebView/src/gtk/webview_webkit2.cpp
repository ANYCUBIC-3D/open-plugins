...
#if USE_WEBVIEW && USE_WEBVIEW_WEBKIT2
...
#include "gtk/webview_webkit.h"
#include "gtk/private/variant.h"
...
#include "wx/base64.h"
#include "wx/log.h"
#include "gtk/private/webview_webkit2_extension.h"
...
{
public:
wxWebViewWebKitHandlerRequest(WebKitURISchemeRequest *request):
        m_request(request),
        m_handler(nullptr),
    ...
    {
        return wxString::FromUTF8(webkit_uri_scheme_request_get_uri(m_request));
    }
    ...
    // 封装错误处理函数
    static void handleStreamError(GInputStream* stream, GError* error) {
        wxLogError("Failed  data from stream: c:%d msg:%s domain:%s",
                   error->code, error->message, g_quark_to_string(error->domain));
        g_error_free(error);
    ...
    virtual wxInputStream* GetData() const override
    {
        if (!m_dataStream)
        {
            GInputStream* stream = webkit_uri_scheme_request_get_http_body(m_request);
            if (stream) {
                ...
                do {
                    ...
                    bytesRead = g_input_stream_read(stream, tmpbuffer, bufferSize, nullptr, &error);
                    if (error) {
                        handleStreamError(stream, error);
                        return nullptr;
                    }
                    buffer.Append(reinterpret_cast<char*>(tmpbuffer), bytesRead);
                    if (bytesRead < bufferSize) {
                        break;
                    }
                } while (true);

                m_dataStream = new wxStringInputStream(buffer);
                g_object_unref(stream);
                return m_dataStream;
            }
        }
    ...
    {
        return wxString::FromUTF8(webkit_uri_scheme_request_get_http_method(m_request));
    }

    virtual wxString GetHeader(const wxString& name) const override
    {
        SoupMessageHeaders* headers = webkit_uri_scheme_request_get_http_headers(m_request);
        auto val =    soup_message_headers_get_one(headers, name.utf8_string().c_str());
        if(val != nullptr){
    ...
    mutable wxStringInputStream* m_dataStream;
};


class wxWebViewWebKitHandlerResponse : public wxWebViewHandlerResponse
{
public:
wxWebViewWebKitHandlerResponse(WebKitURISchemeRequest *request):
    m_request(request),m_status(200)
    {}
    ...
    {
        m_header.emplace(name.utf8_string(), value.utf8_string());
    }
    ...
    private:
        // 私有方法，用于完成响应
        void FinishResponse(GInputStream* stream, gsize length)
        {
            
            // 创建 WebKitURISchemeResponse
            WebKitURISchemeResponse* response = webkit_uri_scheme_response_new(stream, length);
            webkit_uri_scheme_response_set_status(response, m_status, m_reason.utf8_str());
            
            // 设置响应头
            if (!m_header.empty())
            {
                SoupMessageHeaders* header = soup_message_headers_new(SOUP_MESSAGE_HEADERS_RESPONSE);   
                for (auto& _header : m_header) {
                    soup_message_headers_append(header, _header.first.c_str(), _header.second.c_str());
                }
    ...
    public:
        ...
        {
            wxInputStream* IStream = data->GetStream();
            if (!IStream) {
                FinishWithError();
                return;
            }
            ...
            size_t length = IStream->GetLength();
            guint8 *buffer = g_new(guint8, length);
        
            IStream->Read(buffer, length);
        
            // 创建 GInputStream
            GInputStream* stream = g_memory_input_stream_new_from_data(buffer, length, g_free);
        
            // 完成响应
            FinishResponse(stream, length);
        }
    
        virtual void FinishWithError() override
        {
            // 创建 GInputStream
            GInputStream* stream = g_memory_input_stream_new_from_data(m_reason.utf8_str().data(), m_reason.size(), nullptr);
        
            // 完成响应
            FinishResponse(stream, m_reason.size());
        }
...
static void wxgtk_webview_webkit_download_started(WebKitDownload *download, gchar *suggested_filename, wxWebViewWebKit *webKitCtrl)
{
    auto requst = webkit_download_get_request(download);
    auto strUri =wxString::FromUTF8(webkit_uri_request_get_uri(requst));
    wxWebViewEvent evt(wxEVT_WEBVIEW_DOWNLOAD_START, webKitCtrl->GetId(), strUri, "", wxWEBVIEW_NAV_ACTION_OTHER);
    webKitCtrl->GetEventHandler()->ProcessEvent(evt);
    if(!evt.IsAllowed()){
        webkit_download_cancel(download);  ///< 取消下载
        return;
    }
    auto receivedDataLength = webkit_download_get_received_data_length(download);
    wxWebViewEvent startEvent(wxEVT_WEBVIEW_DOWNLOAD_SUGGESTED,  webKitCtrl->GetId(),strUri, wxString(suggested_filename), wxWEBVIEW_NAV_ACTION_OTHER);
    startEvent.SetInt(static_cast<int>( receivedDataLength));
    startEvent.Veto();//先标记成拒绝
    if (webKitCtrl && webKitCtrl->GetEventHandler())
        webKitCtrl->GetEventHandler()->ProcessEvent(startEvent);
    if(startEvent.IsAllowed()){
        wxString strResultFilePath  =  startEvent.GetString();
        webkit_download_set_destination(download,strResultFilePath.utf8_str());
        
    }else{
      webkit_download_cancel(download); ///< 取消下载
    }
}
static void wxgtk_webview_webkit_download_finished(WebKitDownload* download, wxWebViewWebKit*webKitCtrl)
{
    auto requst = webkit_download_get_request(download);
    auto strUri =wxString::FromUTF8(webkit_uri_request_get_uri(requst));
    auto filename = webkit_download_get_destination(download);
    wxWebViewEvent event(wxEVT_WEBVIEW_DOWNLOAD_FINISHED, webKitCtrl->GetId(), strUri, wxString(filename), wxWEBVIEW_NAV_ACTION_OTHER);
    if (webKitCtrl && webKitCtrl->GetEventHandler())
        webKitCtrl->GetEventHandler()->ProcessEvent(event);
}
// static void wxgtk_webview_webkit_download_failed(WebKitDownload* download, GError* error, wxWebViewWebKit*webKitCtrl)
// {
//     g_assert_nonnull(error);
// }

// static void wxgtk_webview_webkit_download_received_data(WebKitDownload* download, guint64 dataLength,  wxWebViewWebKit*webKitCtrl)
// {

// }
static void wxgtk_webview_webkit_download_signal(wxWebViewWebKit*webKitCtrl, WebKitDownload *download){
        // g_signal_connect(download, "received-data", G_CALLBACK(wxgtk_webview_webkit_download_received_data), webKitCtrl);
        g_signal_connect(download, "finished", G_CALLBACK(wxgtk_webview_webkit_download_finished), webKitCtrl);
        // g_signal_connect(download, "failed", G_CALLBACK(wxgtk_webview_webkit_download_failed), webKitCtrl);
        g_signal_connect(download, "decide-destination", G_CALLBACK(wxgtk_webview_webkit_download_started), webKitCtrl);
}
                                           ...
                                           wxWebViewWebKit *webKitCtrl)
{
    ...
    if(handler)
    {
        wxWebViewWebKitHandlerRequest req(request);
        wxSharedPtr<wxWebViewHandlerResponse> resp(new wxWebViewWebKitHandlerResponse(request));
        req.SetHandler(handler.get());
...
static gboolean
wxgtk_webview_webkit_load_failed_with_tls_errors(WebKitWebView*,
                                               gchar* failing_uri,
...
{
    // 从 URI 中提取主机名
    wxURI uri(wxString::FromUTF8(failing_uri));
    wxString hostStr = uri.GetServer();
    const gchar* host = hostStr.utf8_str();

    if (!host || *host == '\0') {
        wxLogError("Failed to extract host from URI: %s", failing_uri);
        return FALSE;
    }

    // 忽略所有 TLS 证书错误
    webkit_web_context_allow_tls_certificate_for_host(
        webkit_web_view_get_context (reinterpret_cast<WebKitWebView* >(webKitCtrl->GetNativeBackend())),
        certificate,
        host
    );
    ...
    // 重新加载页面
    // webkit_web_view_load_uri(webKitCtrl->m_web_view, failing_uri);
                      ...
                      const wxString& name)
{
    ...
    webkit_web_context_set_tls_errors_policy(webkit_web_view_get_context(m_web_view), WEBKIT_TLS_ERRORS_POLICY_IGNORE); //临时禁用证书验证
    
     // 添加TLS错误处理信号
    g_signal_connect(m_web_view, "load-failed-with-tls-errors",
                     G_CALLBACK(wxgtk_webview_webkit_load_failed_with_tls_errors),
                     this);
    ...
    g_signal_connect_swapped(webkit_web_view_get_context(m_web_view), "download-started", 
                     G_CALLBACK(wxgtk_webview_webkit_download_signal), this);
...
void wxWebViewWebKit::ProcessJavaScriptResult(GAsyncResult *res, wxWebKitRunScriptParams* params) const
{
    ...
    if ( js_result )
    {
        ...
        if ( wxGetStringFromJSResult(js_result, &scriptResult) )
        {
            ...
            bool success = ::ExtractOutput(scriptResult, &scriptOutput);
...
void wxWebViewWebKit::SetupWebExtensionServer()
{
    ...
    if (error)
    {
        g_warning("Failed to start web extension server on %s: %s",
                  address.c_str(), error.GetMessage().utf8_string().c_str());
    }
    else
    {
...
#endif // USE_WEBVIEW && USE_WEBVIEW_WEBKIT2
